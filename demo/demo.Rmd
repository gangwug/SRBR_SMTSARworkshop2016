...
* ##### IMPORTANT NOTE: For windows users, it is required to click 'Open in Browser' button for downloading the analysis outputs to local computer. For mac users, it is optional to click 'Open in Browser' button and we suggest not to click this button for this demo.
...
* #### caseA (csv): 1) evenly sampled with 4h interval covering two days (from CT18 to CT62), 2) no replicate or missing value
    ...
    + ##### step 4 (mac users): click 'download' button at bottom to download the outputs to Desktop and name the output file as 'meta2d_caseA.csv'
    ![](../inst/images/3-2CaseA_Step4Mac.png)
    + ##### step 4 (windows users): Change the default download folder to Desktop, and you could follow the introduction for [Chrome](https://support.google.com/chrome/answer/95759?co=GENIE.Platform%3DDesktop&hl=en) or [Firefox](http://audible.custhelp.com/app/answers/detail/a_id/4661/~/how-can-i-change-my-download-location-using-firefox%3F). Click 'download' button at bottom to download the outputs to laptop. There will be a file named as 'meta2d.csv' in the download folder for used web browser. Manually re-name this file as 'meta2d_caseA.csv'.
...
* #### caseC (csv): 1) cell cycle data from yeast, 2) evenly sampled with 16min interval from 2min to 162min, 3) no replicate or missing value, 4) the expected time length for cell cycle of this kind of yeast is 85min
    ...
    + ##### detail steps: 1) follow above cases to upload caseC dataset from 'data' directory, 2) parameters setting: select 'csv' under fileStyle, set 'minper' as 80 and 'maxper' as 96, set 'ARSdefaultPer' as 85, and select all three methods under 'cycMethod', 3) click 'Run' button, 4) click 'Download' button to save output (name it as 'meta2d_caseC.csv'), 5) you could see a window like below during analyzing caseC dataset
    ![](../inst/images/3-2CaseC.png)

### 3.3. Do some practice with MetaCycleApp according to instructions mentioned above. There are five exercise datasets (file names containing 'exercise') in 'data' directory, please analyze exerciseA and exerciseB within this workshop, and do the remaining after this workshop.
...
* ##### general advice: 1) select all three methods under 'cycMethod' and MetaCycleApp will automaticlly select methods that could be used to analyze input dataset, 2) to search circadian profiles, it is better to set both 'minper' and 'maxper' as 24, 3) select 'csv' or 'txt' under 'fileStyle' according to input file, 4)
name outputs as 'meta2d_exercise*'

### 3.4. Analyze one experimental data (it may take several minutes)
...
* ##### general advice: 1) select 'JTK' and 'LS' in analyzing high-resolution datasets (eg. 2h / 2days, 'ARS' shows relative higher false positive rates in analyzing these kinds of datasets), 2) set 'minper' and 'maxper' both as '24' for searching circadian transcripts, 3) save output on Desktop and name it as 'meta2d_experimentA.csv', 4) if following above advice, you could see a window like below during analyzing this dataset

![](../inst/images/3-4ExperimentA.png)

### 3.5. A short discussion part during MetaCycleApp's running process (Dr. John Hogenesch will lead this discussion part)

* #### dicussionA: For studying circadian transcriptome, what is the difference for collecting samples covering one day and two days?

    + ##### datasets: 'dicussionA_1day.csv' and 'dicussionA_2days.csv' are in 'data' directory, which are re-sampled (5000 probesets, 1h / 1day or 1h / 2days) from 'Hughes2009_MouseLiver1h.txt'
...
* #### dicussionB: During the process of experimental design or reaching conclusion from the data, have we thought about the false negative rates?
